...
def main():
    ...
    def write_js(fname, varname, data):
        ...
        def clean(obj):
            ...
            return obj
        path = DIST_DIR / fname
        # Preserve hand-maintained window.ENEMY_STATS block (engine depends on it;
        # regenerating without it deletes ~90 enemy stat entries — see 2026-06-10 incident).
        preserved = ''
        if path.exists():
            existing = path.read_text(encoding='utf-8')
            m = re.search(r'(// ENEMY_STATS:.*?window\.ENEMY_STATS = \{.*?\n\};\n)', existing, re.DOTALL)
            if m:
                preserved = m.group(1)
