...
var HARVEST_CIRCLE_STAGE1_ENRICHED_CHOICES = [
  ...
  // 1. GRAIN MEASURER: QUOTA MANIPULATION
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The land protector works the eastern fields.', skill: 'vigor', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
  ...
  // 2. MARKET BROKER: PRICE MANIPULATION
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The market hall entrance posts the official session notices.', skill: 'wits', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
      ...
      if (result.isCrit) {
        ...
        G.lastResult = `Varik puts his pen down. "Who sent you?" He's not accusing — he's assessing. By the end of the afternoon, the market row has heard that someone came to the broker asking questions about festival pricing on behalf of unnamed parties. Your business here now has a reputation attached to it. The watchful eyes along this row will make the next question twice as hard to ask — and whatever you ask, someone will report it before you've left the stall.`;
  ...
  // 3. PROTECTOR OF MAGICAL FORESTS: LAND CORRUPTION
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The Halversen family farms the eastern border strip.', skill: 'charm', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
      ...
      if (result.isCrit) {
        ...
        G.lastResult = `Elyra answers in environmental terms — soil conditions, weather variance, seasonal barrier load. The words are accurate and the sentences are complete. Nothing in them touches what was asked. She keeps walking at the same pace throughout, maintenance log tucked under her arm, boots marking the damp soil between rows of harvested stakes. The smell of turned earth and cut chaff drifts across the eastern annex. The conversation moves around the subject with a deliberateness that reads less like deflection and more like something practiced until it became automatic.`;
  ...
  // 4. STORAGE KEEPER: GRAIN DIVERSION
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The distribution hall staging log is a different authority.', skill: 'wits', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
      ...
      if (result.isCrit) {
        ...
        G.lastResult = `"Storage records are not for external review." The Storage Keeper doesn't elaborate further and doesn't move. They stay positioned between you and the inventory ledger until you leave. Within the hour the storage authority supervisor has been informed that someone came asking about inventory management. The watchful response to a single inventory question means this is a protected line — coming back will be harder and will draw more attention than the first visit did.`;
        G.worldClocks.watchfulness++;
  ...
  // 5. FESTIVAL COORDINATOR: CELEBRATION MANIPULATION
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The reading room holds the council\'s ceremony approval records.', skill: 'wits', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
      ...
      if (result.isCrit) {
        ...
        G.lastResult = `The Coordinator's hands stop moving at the question — mid-fold, paper held between fingers. "Festival planning is a community matter." They step back from the table without setting the paper down first, which they notice and correct. By the end of the morning, the festival committee chair has been informed that questions about distribution procedures came from outside the commune. The Coordinator does not return to the table while you're still in the room. The pressure of that notification will make the next distribution question harder to ask without being intercepted at the door.`;
        G.worldClocks.pressure++;
        addJournal('Festival Coordinator now protective of festival procedures', 'complication', `harvest-festival-hostile-${G.dayCount}`);
      } else if (result.total >= 12) {
        G.lastResult = `The Coordinator says this season's distribution has been more layered than usual. "There are different tiers now." They don't elaborate on who set the tiers, by what authority, or what the tiers mean in practice for households at the bottom of them. The draft distribution sheet sits face-down on the table between you both, its edges flush with the timber surface. The smell of fresh canvas drifts from the banner poles being set along the granary steps outside. They glance at the sheet once during the conversation and don't touch it. Whatever is on the other side, they've decided you won't see it today.`;
        addJournal('Festival Coordinator confirmed inconsistent festival allocations', 'evidence', `harvest-festival-biased-${G.dayCount}`);
      } else {
        G.lastResult = `The Coordinator says festival planning is handled internally. "The commune manages its own celebration." The distribution sheet stays face-down on the table between you, its edges straight and deliberately untouched. Outside the granary windows, the festival preparation crew sets banner poles along the steps, canvas snapping in the morning air. The answer is short, complete, and closes the subject. Outside questions about inside matters produce a particular kind of silence — not hostile, just sealed, the way a document becomes unreachable when the only hand authorized to open it chooses not to.`;
  ...
  // 6. RECORD KEEPER: DOCUMENT FALSIFICATION
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The public register tracks documentation format changes.', skill: 'wits', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
      ...
      if (result.isCrit) {
        G.lastResult = `Farlan has the evidence already assembled — he's been waiting for someone to ask. "Look at the delivery record for the Coalward family, third week of last season." He sets it beside a summary sheet on the reading room table, the chalk dust of the tally marks faintly visible at the sheet's margin. The delivery figure on the record is higher than the summary confirms. "That change happened after the document was signed. The ink shade is different — look at the column edge." He stacks the flagged documents. "Grain delivered in one quantity, recorded in another. The recordkeeping is supposed to prevent this. Someone changed what goes into the records, and they did it after the original was signed."`;

        G.stageProgress[1]++;
        addJournal('Record Keeper revealed document falsification system', 'evidence', `harvest-records-${G.dayCount}`);
      } else if (result.isFumble) {
        G.lastResult = `Farlan straightens in his chair, spine finding the seat back. "Harvest documentation is internal academic material. External review is not appropriate." He closes the folder in front of him with both hands, a deliberate gesture. Dust rises from the cover. The conversation has ended on grounds he finds professionally sufficient — he doesn't reach for the next file until you've taken a step toward the door, as if confirming the direction before returning to work. The watchful stillness of that wait means your name and purpose are already noted in the way that moves between desks.`;
        G.worldClocks.watchfulness++;
        ...
        G.lastResult = `Farlan answers in procedural terms. Documentation requires training and authorization to interpret correctly — that much is genuine, and the reading room's smell of old paper and cedar shelving lends the claim an air of permanence. The folder stays on the table between his hands, closed, the chalk tally marks on the cover faint from prior handling. He isn't wrong. He's also performing a competence barrier the same way a door performs a lock: technically neutral, constructed from real knowledge, and functionally final as long as no one holds the key he isn't showing.`;
  ...
  // 7. INSPECTOR: QUALITY SUPPRESSION
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The compliance records are open during the inspector\'s afternoon rounds.', skill: 'vigor', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
      ...
      if (result.isCrit) {
        G.lastResult = `Garren keeps his voice low and his back to the lantern above the grain scales. "I mark the grain. I file the mark. Two days later I check the record and the mark is gone — passed, no note, nothing." He holds up his inspection log beside the official record; the grain store smells of chaff and timber. Same batch, two different outcomes, columns side by side. "I've been told directly: connected-family grain passes. I had a batch last month, Halversen family — quality grain — flagged for minor moisture variance. Supervisor overruled it." He sets both logs flat. "The grain from that Halversen flag was good. The batch that passed without a flag — I won't eat from it."`;

        G.stageProgress[1]++;
        addJournal('Inspector revealed corrupted quality system', 'evidence', `harvest-quality-${G.dayCount}`);
      } else if (result.isFumble) {
        G.lastResult = `Garren's hands go still over the inspection log — a particular stillness, deliberate. "Quality inspection is internal process." He steps back from the exchange. The lantern at his station throws the same light it always does; the grain bins smell of chaff and dried straw. By midday the inspection supervisor has been notified that a stranger came asking about assessment procedures. When you pass Garren in the afternoon, he holds his log against his chest and looks somewhere past your shoulder. The pressure of being logged here will make the next inspection inquiry harder to open without the supervisor arriving first.`;
        G.worldClocks.pressure++;
        ...
        G.lastResult = `Garren keeps the inspection terminology precise and the conversation technical. Moisture variance, batch cycling, storage grade — every answer is accurate and none of it addresses what matters. The lantern above the grain scales throws its same steady light on the inspection station. The log stays under his hand, palm flat against the cover. He isn't protecting himself from you. He's protecting himself from whoever reads the log after you've gone — and that person, whoever they are, has already made clear they read everything carefully.`;
  ...
  // 8. DISPUTE MEDIATOR: CONFLICT RESOLUTION FAILURES
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The Halversen family knows what the mediator ruled against them.', skill: 'charm', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
  ...
  // 9. GRAIN SYSTEM HIERARCHY: AUTHORITY REORGANIZATION
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The elder council\'s posted session minutes go back eighteen months.', skill: 'wits', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
  ...
  // 10. FARMING NETWORKS: FAMILY DISPLACEMENT
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The appeal filing records are public and held separately from the roster.', skill: 'wits', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
      ...
      if (result.isCrit) {
        ...
        G.lastResult = `A family liaison intercepts the inquiry at the records desk, arriving before the archivist has finished pulling the second volume — unhurried. "Family roster information is not available for external research." Your name is logged in a column you didn't see until the pen was already moving, the scratch of the nib audible in the quiet archive room. By the following morning, three farming family elders have been notified that someone came asking about the composition of their network. The records go back on the shelf. The archivist doesn't meet your eyes. You've been noticed here in a way that travels further and faster than you did.`;
        G.worldClocks.watchfulness++;
        addJournal('Family authorities alerted to network analysis', 'complication', `harvest-network-alert-${G.dayCount}`);
      } else if (result.total >= 12) {
        G.lastResult = `The roster shows movement — names crossed out, names added in the same careful hand. The exits are documented under administrative categories that don't invite follow-up questions: storage dispute resolution, quota non-compliance, voluntary withdrawal. The entries are recent and all share a similar profile in the affiliation column. The archive room smells of old paper and dry cedar shelving; the second volume sits half-pulled from the shelf where the archivist paused. The composition has shifted in a consistent direction, but whether deliberately requires checking what those administrative categories actually mean in practice.`;
  ...
  // 11. SPOILAGE SYSTEM: DEGRADATION ACCELERATION
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The maintenance schedule at the yard entrance is public-facing.', skill: 'vigor', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
      ...
      if (result.isCrit) {
        ...
        G.lastResult = `A storage supervisor appears at the annex door while you're comparing the two sections. "This area requires keeper authorization." You're walked out and your name is added to the storage authority incident log. Someone with a clipboard makes a note. The clipboard goes with the supervisor. The watchful response to storage comparison means the spoilage differential is a protected question — the next attempt to access these sections will arrive with a supervisor already waiting.`;
        G.worldClocks.watchfulness++;
        addJournal('Storage authorities alerted to spoilage analysis', 'complication', `harvest-spoilage-alert-${G.dayCount}`);
      } else {
        G.lastResult = `The eastern annex grain is degrading faster than the western. The conditions differ visibly — the eastern floor holds moisture where the western doesn't, the pest traps here are old and unserviced, the air circulation dead in the corners where the grain sacks sit lowest. The smell of slow spoilage is damp and sweet at the annex threshold. Whether the gap is neglect or design isn't answerable from standing in the doorway. But the pattern lines up with family affiliation in a direction that plain neglect usually doesn't sustain over two full seasons.`;
  ...
  // 12. FAMILY POWER TRACKING: WHO'S RISING
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The personnel register holds the council chair replacement record.', skill: 'wits', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
      ...
      if (result.isCrit) {
        ...
        G.lastResult = `A patron-family representative approaches before you've finished the second record review — not hurrying, arriving at the pace of someone who knows exactly how long the walk from their office takes. "What is the purpose of this research?" The question is calm and specific, the kind that assumes the answer is already written down somewhere. Your name is in three logs by the following morning, each entry brief, none of them hostile in phrasing. The pressure of those entries is administrative — quiet, cumulative, harder to argue against than a direct refusal.`;
        G.worldClocks.pressure++;
        addJournal('Patron families alerted to power analysis', 'complication', `harvest-power-alert-${G.dayCount}`);
      } else if (result.total >= 12) {
        G.lastResult = `Two families have moved from the roster margins to governance positions in under two years. Their ascent shows up in the appointment records as a sequence of ratified nominations, each one unremarkable on its own — the kind that fills the tally column without drawing comment. The reading room holds the appointment ledger under a pine-frame cabinet; the chalk marks on the tally board above it record the current session schedule. What the record doesn't show is who proposed them — that detail belongs in the closed session minutes, which are held under separate authority and aren't accessible through this desk.`;
        addJournal('Family authority advancement changes confirmed', 'evidence', `harvest-power-confirmed-${G.dayCount}`);
      } else {
        G.lastResult = `The governance record shows the current power arrangement but not how it got there. Internal patron-family politics don't surface in the appointment logs — those decisions happen in rooms before they're committed to paper, and what lands in the record is the outcome, not the process that produced it. The appointments look unanimous; each ratification stamp is clean, the ink consistent, the column margins uncluttered. The record of who proposed them isn't here. That would be in closed session minutes held under a different clerk's authority, and those minutes require authorization to open. The thread points the same direction it has been pointing — the chair who ratified those appointments was replaced the month after.`;
  ...
  // 13. QUOTA DEPENDENCY: CONTROL SYSTEM
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The quota intake form lays out the credit terms plainly.', skill: 'wits', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
  ...
  // 14. FESTIVAL MANIPULATION: UNITY WEAPONIZATION
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The archive holds four seasons of ceremony scripts on a different access authority.', skill: 'wits', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
  ...
  // 15. FIELD RUMOR: FARMER WHISPERS
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The communal fire draws crews together after the authority building closes.', skill: 'charm', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
  ...
  // 16. MARKET RUMOR: MERCHANT WHISPERS
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The festival square stalls in the late afternoon draw less careful conversation.', skill: 'charm', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
  ...
  // 17. INSTITUTIONAL CRACK: CORRUPTION PROOF COMPILATION
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The outbound staging area receives outside courier traffic twice weekly.', skill: 'vigor', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
      ...
      if (result.isCrit) {
        ...
        G.lastResult = `The individual pieces each make sense on their own: quota irregularities, spoilage disparity, falsified delivery figures, festival language drafted before the council saw it. Laid across the work table, the smell of old paper and grain dust from the archive visit still on the documents, they suggest coordination. The timing too consistent, the direction too uniform, the institutions too varied for one family acting alone. The final link between the evidence and a single coordinating hand remains the gap. Close it and the paper trail becomes a mechanism, not just a pattern.`;
  ...
  // 18. MORAL PRESSURE: FAMILY LOYALTY COMPROMISE CHOICE
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The weighing station is unwatched in the early morning before the authority shift begins.', skill: 'charm', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
      ...
      G.lastResult = `You find ${npc.name} at the weighing station before the morning authority shift begins, the tally boards still blank, the granary steps empty behind them. They crumble quickly — the words coming out in the wrong order, then the right one. "${npc.fear}" The fear behind it is specific and old, the kind worn smooth by repetition. They're trapped, complicit, and frightened. A Harvest Circle record-keeper stands twenty feet away, back turned, making entries in the chalk tally. The next step belongs to you: protect this person and keep the inquiry quiet, or expose their role and risk losing the only insider access still available to you.`;
  ...
  // 19. DISCOVERY MOMENT: EXTERNAL COORDINATION SOURCE
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The Iron Compact liaison tracks cargo provenance the manifests obscure.', skill: 'wits', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
  ...
  // 20. CONSPIRACY COMPLETE: EXTERNAL INFLUENCE CONFIRMATION
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'A formal Compact inquiry opens the Provision Compact transaction records.', skill: 'wits', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1);
      G.telemetry.turns++;
      ...
      if (result.isCrit) {
        ...
        G.lastResult = `As you prepare to confirm the complete conspiracy, you're stopped directly by coordinated authorities. They make it abundantly clear that knowledge of the full scope will result in permanent removal or elimination. You've understood the scale of the conspiracy, but speaking it aloud will make you expendable. The pressure from this point forward is not institutional — you are now tracked at the level where documentation becomes the only defense.`;
  ...
  // 22. CLUE: CHARTER MARK CONTAINER
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The staging shed holds the crew lead\'s handoff carbons through end of week.', skill: 'vigor', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
  ...
  // 23. ARCHETYPE-GATED: READING HARVEST CIRCLE
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The night shift crew sees the full staging area before section leads divide it.', skill: 'charm', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      ...
      if (arch === 'combat') {
        G.lastResult = `The field organization has changed since prior seasons — the rows run in sections with specific handoff points between them, staked at the corners with painted markers. Not efficient for harvest yield, but efficient for oversight: no individual worker moves between sections, no one accumulates a running count. The smell of turned earth and chaff carries across the morning air as the first crew takes the row. Someone reorganized this fieldwork so that each worker sees only their section and no complete tally ever forms below a section lead. The organization is designed to prevent workers from knowing the full production number.`;
      } else if (arch === 'magic') {
        G.lastResult = `Two fields with identical soil density and planting depth are being harvested at different rates. The western field moves deliberately — careful cutting, bundled and stacked for maximum grain retention. The eastern field moves quickly, speed over yield, stalks cut lower and left in longer rows. Dawn light catches the dew on both sections equally; the differential isn't in the crop. The eastern field's output is moving under different time pressure toward a different destination. One harvest, two paces, two places the grain ends up.`;
      } else if (arch === 'stealth') {
        G.lastResult = `A separate cart leaves the staging area every morning before the main convoy departs — no banner, no Provision Compact mark on the side panel, the driver not wearing the standard allocation crew jacket. It doesn't route to the central weighing station. It takes the northern lane toward the storage facility that feeds directly into the suspect routing chain. The separation happens before any official weighing begins, before any count is committed to paper. The laundered volume is pre-staged while the tally boards at the main station are still blank.`;
      } else {
        G.lastResult = `A field overseer argues with a worker at the stake line separating two sections. The worker says their section met the morning target; the overseer reads from a document that says otherwise. The worker is right — the row count is visible from where you stand, the bundled sheaves still stacked along the boundary marker. The overseer's document doesn't match the physical output in front of both of them. Production records are being adjusted at the point of collection, before the numbers travel up to the weighing station tally board.`;
      }
  ...
  // 25. ATMOSPHERE: THE GRANARY STEPS AT MIDDAY
  {
    ...
    failResult: {
      ...
      xp: 0,
      effects: [],
      next: [{text: 'The evening distribution is smaller and less managed than midday.', skill: 'wits', tag: 'safe', align: 'neutral', cid: '__arrive__'}]
    },
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
  ...
  // 28. SHADOW RIVAL INTRO
  {
    ...
    fn: function() {
      ...
      const arch = G.archetype && G.archetype.group;
      if (arch === 'combat') {
        G.lastResult = `"Carried no visible weapons but moved like someone who expected to need them," the driver says, settling his weight onto the staging area fence post. "Asked about the security on the northern leg — not what we were carrying, specifically. Whether anyone was watching the route. Whether the overnight stop had a Compact watchman posted." The driver pauses. "Didn't ask about cargo weights or routing numbers. Just the corridor's vulnerabilities." Someone is assessing the transit route's exposure, not the documentation discrepancy. Planning for movement, not evidence.`;
      } else if (arch === 'magic') {
        G.lastResult = `"Had a small instrument they used at the staging area before we departed — pulled it out near the charter-marked container for maybe thirty seconds," the driver says. The morning light here is the flat gray before the sun clears the granary roof; the driver watches the staging crew without looking directly at them. "Checked three containers without touching any of them. Put it away, made a note, didn't say what they found." A trace reading at the staging area, characterizing what passed through before the convoy formed. Working from physical evidence, not paper records.`;
      } else if (arch === 'stealth') {
        G.lastResult = `"Never asked the same question twice," the driver says. "I noticed because I've had investigators travel with us before — they always circle back to what they missed. This one moved forward, never doubled." The driver's hands rest on his knees; the morning distribution crew moves past without slowing. "One question per person, one answer gathered, then they were done with that source." Professional non-redundant extraction. Maximum information per exchange, no telegraphing of gaps. A trained information gatherer who already knows how to move through a territory without leaving a shape behind.`;
      } else {
        G.lastResult = `"Bought food for the whole convoy," the driver says. "Generous. Said it was company policy — they always fed the people they worked alongside on the road." He shrugs. "And everyone talked more than they should have. A long drive, a warm meal, a generous stranger. Generous gestures are very effective when you have eight hours until the next stop." Social infrastructure deployed before any question was asked, the cooperation funded in advance. They were ahead of their own inquiry before it started.`;
      }
  ...
  },
{
  ...
  fn: function() {
    ...
    if (G.flags[key]) {
      G.lastResult = 'The board has nothing new since this morning. A yield quota notice is pinned at the center, its edges lifting in the draft from the granary door — the same sheet that was there at first light, the ink gone dry and the chalk tally mark below it unchanged. A market stall assignment from the prior day hangs beside it at a slight tilt, nobody having straightened it. The smell of damp grain dust drifts from the storage side of the building. The tally board at the weighing station end posts the morning count each day and would have the current numbers; this board will not.';
      return;
    }
    G.flags[key] = true;
