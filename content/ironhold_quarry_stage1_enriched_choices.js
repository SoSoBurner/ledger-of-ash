...
var IRONHOLD_QUARRY_STAGE1_ENRICHED_CHOICES = [
  ...
  // 1. FIRST ENCOUNTER: VELKA IRONSPIKE
  {
    ...
    fn: function() {
      ...
      gainXp(34, 'meeting Velka Ironspike');
      if (!G.flags) G.flags = {};
  ...
  // 2. CLUE: THE EASTERN FACE EXTRACTION
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(37, 'reading eastern face extraction records');
  ...
  // 3. CLUE: DARIAN'S OPERATIONS SCHEDULE
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(36, 'comparing operations schedules');
      if (!G.investigationProgress) G.investigationProgress = 0;
      if (!G.worldClocks) G.worldClocks = {};
      G.investigationProgress++;
      if (G.investigationProgress === 3) G.worldClocks.watchfulness = (G.worldClocks.watchfulness || 0) + 1;
  ...
  // 4. INVESTIGATION: THE SECONDARY MINERAL
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(37, 'identifying secondary mineral');
      if (!G.investigationProgress) G.investigationProgress = 0;
      if (!G.worldClocks) G.worldClocks = {};
      G.investigationProgress++;
      if (G.investigationProgress === 3) G.worldClocks.watchfulness = (G.worldClocks.watchfulness || 0) + 1;
  ...
  // 5. ARCHETYPE-GATED: READING THE QUARRY
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(33, 'reading quarry operations');
  ...
  // 7. SOCIAL: DARIAN'S BACKGROUND
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(32, 'researching Darian\'s background');
  ...
  // 8. MORAL PRESSURE: VELKA'S DILEMMA
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(32, 'Velka evidence decision');
      if (!G.flags) G.flags = {};
  ...
  // 9. INVESTIGATION: THE CONTAINER DESTINATION
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(36, 'tracking container destination');
      if (!G.investigationProgress) G.investigationProgress = 0;
      if (!G.worldClocks) G.worldClocks = {};
      G.investigationProgress++;
      if (G.investigationProgress === 3) G.worldClocks.watchfulness = (G.worldClocks.watchfulness || 0) + 1;
  ...
  // 10. ATMOSPHERE: THE QUARRY AT DAWN
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(25, 'observing quarry dawn operations');
  ...
  // 11. INVESTIGATION: QUARRY OWNERSHIP STRUCTURE
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(35, 'mapping quarry ownership structure');
      if (!G.investigationProgress) G.investigationProgress = 0;
      if (!G.worldClocks) G.worldClocks = {};
      G.investigationProgress++;
      if (G.investigationProgress === 3) G.worldClocks.watchfulness = (G.worldClocks.watchfulness || 0) + 1;
  ...
  // 12. PERSONAL ARC: VELKA'S LOG OFFSITE
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(34, 'securing Velka\'s log offsite');
      if (!G.flags) G.flags = {};
  ...
  // 13. RUMOR LAYER
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(30, 'gathering quarry worker rumors');
  ...
  // 14. SOCIAL: THE RETURNING EASTERN FACE WORKER
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(33, 'interviewing eastern face worker');
  ...
  // 15. INVESTIGATION: THE QUOTA TARGET
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(36, 'estimating extraction quota completion');
      if (!G.investigationProgress) G.investigationProgress = 0;
      if (!G.worldClocks) G.worldClocks = {};
      G.investigationProgress++;
      if (G.investigationProgress === 3) G.worldClocks.watchfulness = (G.worldClocks.watchfulness || 0) + 1;
  ...
  // 16. ATMOSPHERE: VELKA'S TWENTY YEARS
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(24, 'understanding Velka\'s perspective');
  ...
  // 17. INVESTIGATION: CONNECTING IRONHOLD TO SHELKOPOLIS
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(35, 'connecting Ironhold to central investigation');
      if (!G.investigationProgress) G.investigationProgress = 0;
      if (!G.worldClocks) G.worldClocks = {};
      G.investigationProgress++;
      if (G.investigationProgress === 3) G.worldClocks.watchfulness = (G.worldClocks.watchfulness || 0) + 1;
  ...
  // 18. WORLD COLOR: THE QUARRY FACE
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(24, 'observing active quarry extraction');
  ...
  // 19. INVESTIGATION: TIMELINE URGENCY
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(36, 'assessing post-extraction timeline');
  ...
  // TYPE: DISCOVERY — WORLD COLOR VIGNETTE
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(19, 'observing quarry acoustic change');
  ...
  // TYPE: DISCOVERY — ARCHETYPE GATE (Support family)
  {
    ...
    fn: function() {
      ...
      if (family !== 'Support') {
        ...
        gainXp(16, 'noting eastern face worker fatigue pattern');
        G.recentOutcomeType = 'observe'; maybeStageAdvance(); return;
      }
      gainXp(36, 'assessing eastern face worker health pattern');
      G.stageProgress[1]++;
  ...
  // TYPE: DISCOVERY — BACKGROUND FLAVOR
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(29, 'reading equipment shed wall chart');
  ...
  // TYPE: DISCOVERY — RISKY
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(34, 'sampling eastern drainage channel');
  ...
  // TYPE: DISCOVERY — BOLD
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(39, 'tracking ghost container batch');
      const result = rollD20('wits', (G.skills.wits || 0));
      if (result.total >= 14) {
  ...
  // TYPE: DISCOVERY — SAFE/LORE
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(27, 'reviewing Ironhold extraction license');
  ...
  // TYPE: DISCOVERY — SOCIAL/RISKY
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(34, 'intercepting the night delivery contractor');
      const result = rollD20('finesse', (G.skills.finesse || 0));
      if (result.total >= 12) {
  ...
  // TYPE: DISCOVERY — ATMOSPHERE SAFE
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(17, 'reading original quarry depth markers');
  ...
  // TYPE: DISCOVERY — BOLD/LORE
  {
    ...
    fn: function() {
      advanceTime(1); G.telemetry.turns++; G.telemetry.actions++;
      gainXp(37, 'reading geological survey with redaction');
      const result = rollD20('wits', (G.skills.wits || 0));
      if (result.total >= 13) {
