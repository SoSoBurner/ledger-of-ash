...
const SOREHEIM_PROPER_TO_SHELK_ARC = [
  ...
  // 1. DECIDING TO LEAVE
  {
    ...
    fn: function() {
      ...
      addJournal('Left Soreheim with quota evidence', 'field_note', `soreheim-departure-${G.dayCount}`);
    }
  },
  ...
  // 3. ROAD ENCOUNTER: DISPLACED WORKER
  {
    ...
    fn: function() {
      ...
      if (result.total >= target) {
        ...
        addJournal('Ossa: dismissals follow identical template across shifts', 'fact', `soreheim-ossa-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
  ...
  // 4. PATTERN RECOGNITION (available when investigationProgress >= 5)
  {
    ...
    fn: function() {
      ...
      addJournal('Workforce clearance is pre-operational preparation', 'fact', `soreheim-pattern-${G.dayCount}`);
      G.recentOutcomeType = 'success';
    }
  },
  ...
  // 5. MIDPOINT: COURIER CONTACT ON THE SOUTHERN ROAD
  {
    ...
    fn: function() {
      ...
      if (result.total >= target) {
        ...
        if (!G.worldClocks) G.worldClocks = {};
        G.worldClocks.watchfulness = (G.worldClocks.watchfulness || 0) + 1;
        addJournal('False courier on the southern road — route is being monitored', 'fact', `soreheim-arc-courier-${G.dayCount}`);
        G.recentOutcomeType = 'complication';
      } else {
  ...
  // 6. ROAD CAMP: IRON COMPACT LOGISTICS OFFICER
  {
    ...
    fn: function() {
      ...
      if (result.isCrit) {
        ...
        if (!G.investigationProgress) G.investigationProgress = 0;
        G.investigationProgress++;
        addJournal('Vael: undeclared weight moving south matches quota anomaly dates', 'fact', `soreheim-arc-vael-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else if (result.total >= target) {
  ...
  // 7. OUTER DISTRICT THRESHOLD
  {
    ...
    fn: function() {
      ...
      if (result.total >= target) {
        ...
        addJournal('Outer district atmospheric gradient — early stage per Dawnsilk report pattern', 'fact', `soreheim-arc-gradient-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
  ...
  // 8. FIRST CONTACT: SHELKOPOLIS INVESTIGATION NETWORK
  {
    ...
    fn: function() {
      ...
      if (result.total >= target) {
        ...
        if (!G.worldClocks) G.worldClocks = {};
        G.worldClocks.pressure = (G.worldClocks.pressure || 0) + 1;
        addJournal('Dret: Shelkopolis network active, three prior arrivals before you', 'fact', `soreheim-arc-dret-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
  ...
  // 9. SHELKOPOLIS ORIENTATION: THE IRONSPOOL WARD
  {
    ...
    fn: function() {
      ...
      addJournal('Ironspool Ward: dome infrastructure convergence point', 'fact', `soreheim-arc-ironspool-${G.dayCount}`);
      G.recentOutcomeType = 'neutral';
      if (!G.investigationProgress) G.investigationProgress = 0;
      G.investigationProgress++;
    }
  ...
  // 12. ARRIVAL IN SHELKOPOLIS
  {
    ...
    fn: function() {
      ...
      if (!G.investigationProgress) G.investigationProgress = 0;
      G.investigationProgress++;
      addJournal('Arrived in Shelkopolis from Soreheim Proper — Stage 2 begins', 'fact', `soreheim-arc-finale-${G.dayCount}`);
      G.recentOutcomeType = 'success';
      maybeStageAdvance();
    }
  }
