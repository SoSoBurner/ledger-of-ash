...
const WHITEBRIDGE_COMMUNE_STAGE1_ENRICHED_CHOICES = [
  ...
  // 15. SOCIAL: THE FORMER BRIDGE DIRECTOR
  {
    ...
    fn: function() {
      ...
      const result = rollD20('persuasion', (G.skills.persuasion || 0) + Math.floor(G.level / 3));
      if (result.total >= 11) {
        G.lastResult = `Ashe lives on the far side of the crossing now, in a two-room dwelling with a window that faces away from the bridge. "After the inquiry closed, I was told the crossings were authorized under a program outside my clearance level." She pours water for both of you, doesn't drink hers. "I sat with that for four weeks. Then I worked out: I'm the bridge director. There is no program I don't have clearance for. So either I was cut out of my own jurisdiction deliberately, or the program is fiction." She handed in her post rather than sign another month of reports under someone else's authority.`;
        if (!G.flags) G.flags = {};
