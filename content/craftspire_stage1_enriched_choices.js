...
var CRAFTSPIRE_STAGE1_ENRICHED_CHOICES = [
  ...
  // 3. CLUE: WHAT THE EXTRACTED COMPOUND DOES
  {
    ...
    fn: function() {
      ...
      if (!G.flags) G.flags = {};
      if (G.flags.met_jorin) {
        G.lastResult = `Jorin takes his reference texts down from the shelf — three volumes, the kind that live on a work bench rather than a display shelf — and goes through them methodically. Twenty minutes of quiet. The forge outside runs steady. Finally he closes the last volume and puts his hands flat on the table. "Two categories. District reservoir filtration at industrial scale, or precursor synthesis for reactive compounds." He marks the volume page. "If it's filtration, whoever has this needs access to civic conduit infrastructure I've never seen outside a major city. If it's synthesis —" He stops. He marks the page anyway. His handwriting is very precise.`;
        G.flags.identified_compound_purpose = true;
        addJournal('Extracted compound: accumulation volume consistent with district reservoir filtration use OR reactive compound synthesis — industrial-scale implication', 'evidence', `craftspire-compound-purpose-${G.dayCount}`);
      } else {
  ...
  // 17. INVESTIGATION: ACCUMULATION VOLUME ESTIMATE
  {
    ...
    fn: function() {
      ...
      if (G.flags.met_jorin) {
        const result = rollD20('spirit', (G.skills.spirit || 0));
        if (result.total >= 12) {
          G.lastResult = `Jorin's six months plus the other two workshops' data: approximately 340 kilograms of concentrated compound extracted. At reservoir filtration application rates, this is enough to treat fifteen to twenty district-scale reservoirs. At reactive compound synthesis rates, Jorin goes quiet for a long time. "That's not a small operation," he finally says. "Whatever they're building, they've been preparing for it for at least six months and they have industrial-scale capacity." He looks disturbed. "This needs to go to Shelkopolis. This is bigger than Craftspire."`;
          G.flags.calculated_accumulation = true;
          addJournal('340kg extracted over 6 months: enough for 15-20 district reservoirs or large-scale reactive synthesis — Jorin confirms this needs Shelkopolis', 'discovery', `craftspire-volume-${G.dayCount}`);
        } else {
