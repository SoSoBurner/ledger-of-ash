...
const PANIM_HAVEN_TO_SHELK_ARC = [
...

  {
    ...
    fn: function() {
      ...
      addJournal('Left Panim Haven following Bureau case and Tazren\'s trail inland', 'field_note', `panim-arc-departure-${G.dayCount}`);
    }
  },
...

  {
    ...
    fn: function() {
      ...
      addJournal('Sera in Shelkopolis — Case 4-Reckoning record held by Sana in Verdant Row', 'fact', `panim-arc-sera-${G.dayCount}`);
      G.recentOutcomeType = 'success';
    }
  },
...

  {
    ...
    fn: function() {
      ...
      addJournal('Tazren found the container timeline overlap — case suppressed because he was right', 'fact', `panim-arc-sequence-${G.dayCount}`);
      G.recentOutcomeType = 'success';
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.total >= 13) {
        ...
        if (!G.investigationProgress) G.investigationProgress = 0;
        G.investigationProgress++;
        addJournal('Bureau reopened Tazren\'s case under new number — inside contact still working', 'fact', `panim-arc-reopen-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      addJournal('Shelkopolis central administrative archive — Panim Bureau suppressed record may exist here', 'fact', `panim-arc-admin-${G.dayCount}`);
      G.recentOutcomeType = 'neutral';
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.total >= 11) {
        ...
        if (!G.worldClocks) G.worldClocks = {};
        G.worldClocks.pressure = (G.worldClocks.pressure || 0) + 1;
        addJournal('Sana: full Case 4-Reckoning record — 23 documents, Tazren\'s 6 final filings included', 'fact', `panim-arc-sana-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (!G.investigationProgress) G.investigationProgress = 0;
      G.investigationProgress++;
      addJournal('Arrived in Shelkopolis from Panim Haven — Stage 2 begins', 'fact', `panim-arc-finale-${G.dayCount}`);
      G.recentOutcomeType = 'success';
      maybeStageAdvance();
    }
  }
