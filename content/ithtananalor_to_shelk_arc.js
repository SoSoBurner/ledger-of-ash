...
const ITHTANANALOR_TO_SHELK_ARC = [
...

  {
    ...
    fn: function() {
      ...
      addJournal('Left Ithtananalor carrying ghost account transaction pattern for central registry', 'field_note', `ithtan-arc-departure-${G.dayCount}`);
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.total >= 11) {
        ...
        addJournal('Maret\'s authorization revoked after inquiry — carrying findings in her head, Shelkopolis contact ready', 'fact', `ithtan-arc-maret-${G.dayCount}`);
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 11) {
        ...
        if (!G.investigationProgress) G.investigationProgress = 0;
        G.investigationProgress++;
        addJournal('Ghost accounts funded the efficiency review that reduced Lysel\'s garrison — financial and physical operation coordinated', 'fact', `ithtan-arc-lysel-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      addJournal('Ghost accounts created complicity — officials who processed them couldn\'t report without self-implicating', 'fact', `ithtan-arc-leverage-${G.dayCount}`);
      G.recentOutcomeType = 'success';
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.total >= 13) {
        ...
        if (!G.investigationProgress) G.investigationProgress = 0;
        G.investigationProgress++;
        addJournal('Road exchange house: same ghost account pattern — capital already in Shelkopolis financial system', 'fact', `ithtan-arc-exchange-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 12) {
        ...
        addJournal('Calver: mobile asset posted before/after ghost account cycles — currently in Shelkopolis', 'fact', `ithtan-arc-calver-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      addJournal('Central archive: cross-locality registry — ghost account pattern will be visible if systematic', 'fact', `ithtan-arc-archive-${G.dayCount}`);
      G.recentOutcomeType = 'neutral';
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.total >= 11) {
        ...
        if (!G.worldClocks) G.worldClocks = {};
        G.worldClocks.pressure = (G.worldClocks.pressure || 0) + 1;
        addJournal('Hess: independent archive access filed — suppression now creates its own record', 'fact', `ithtan-arc-hess-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (!G.investigationProgress) G.investigationProgress = 0;
      G.investigationProgress++;
      addJournal('Arrived in Shelkopolis from Ithtananalor — Stage 2 begins', 'fact', `ithtan-arc-finale-${G.dayCount}`);
      G.recentOutcomeType = 'success';
      maybeStageAdvance();
    }
  }
