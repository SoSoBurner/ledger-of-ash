...
// D5: Shadowhands handler offers genuine protection with binding obligation
{
  ...
  fn: function() {
    var roll = rollD20('persuasion', G.skills.persuasion);
    if (roll.total >= 14) {
      addNarration('The Offer', 'The handler meets the player at the far end of the Cosmouth public dock, where the evening wind off the water makes it hard to hear. He doesn\'t introduce himself. Roaz considers it useful to both parties for the player to remain operational. In practice: one safe house in Shelkopolis, one in Cosmouth, passage documents under a secondary name, and a standing arrangement that ensures the Pattern Anomaly Notices don\'t get renewed. The cost: one request, at a time of Roaz\'s choosing, details withheld until the moment. He leaves the player one night to decide.');
...
// D7: New NPC with insider Compact knowledge approaches
{
  ...
  fn: function() {
    var roll = rollD20('lore', G.skills.lore);
    if (roll.total >= 14) {
      addNarration('The Cartographer', 'The woman gives her name as Tessaly — cartographic services, Compact infrastructure division, genuinely boring work. She has been waiting at the waystation for two days. She knows the player\'s route pattern, the charter amendment series, and the name of the subsidiary that doesn\'t appear in the public registry. That last one she says quietly, watching the player\'s face. She\'s been inside the administration long enough to know when a pattern is being managed rather than forming by accident. She has a transit map annotated in a personal cipher, written four years ago, before she understood what the routes would eventually document.');
