...
const MIMOLOT_ACADEMY_TO_SHELK_ARC = [
...

  {
    ...
    fn: function() {
      ...
      addJournal('Left Mimolot with formula fragments in academic notation — readable only to trained chemists', 'field_note', `mimolot-arc-departure-${G.dayCount}`);
    }
  },

  {
    ...
    fn: function() {
      ...
      addJournal('Brevard Ashe transferred to Shelkopolis central Academy — will monitor incoming research', 'fact', `mimolot-arc-ashe-${G.dayCount}`);
...

  {
    ...
    fn: function() {
      ...
      addJournal('Craftspire extraction = 340kg = Memory Hall deployment threshold — academic and operational connected at design', 'fact', `mimolot-arc-threshold-${G.dayCount}`);
      G.recentOutcomeType = 'success';
    }
  },

  {
    ...
    fn: function() {
      ...
      if (!G.investigationProgress) G.investigationProgress = 0;
      G.investigationProgress++;
      addJournal('Doss in Shelkopolis for 3 months — forced sabbatical after reclassification query, may have been found', 'fact', `mimolot-arc-doss-location-${G.dayCount}`);
      G.recentOutcomeType = 'success';
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.total >= 13) {
        ...
        if (!G.investigationProgress) G.investigationProgress = 0;
        G.investigationProgress++;
        addJournal('Ashe denied publication then classified formula 14 months ago — suppression started before operation', 'fact', `mimolot-arc-ashe-history-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      addJournal('Shelkopolis central Academy review board includes Warden Order liaison — Ashe now holds that seat', 'fact', `mimolot-arc-academy-${G.dayCount}`);
      G.recentOutcomeType = 'neutral';
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.total >= 12) {
        ...
        addJournal('Doss: full synthesis model complete — lethal at 180+ hours sustained exposure', 'fact', `mimolot-arc-doss-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (!G.investigationProgress) G.investigationProgress = 0;
      G.investigationProgress++;
      addJournal('Arrived in Shelkopolis from Mimolot Academy — Stage 2 begins', 'fact', `mimolot-arc-finale-${G.dayCount}`);
      G.recentOutcomeType = 'success';
      maybeStageAdvance();
    }
  }
