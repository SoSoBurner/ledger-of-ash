...

  {
    ...
    fn: function() {
      ...
      if (result.isCrit) {
        ...
        addJournal('investigation', 'Charter-exempt cargo weight mismatch — no-questions protocol cited', `guild-luthen-partial-${G.dayCount}`);
      }
      G.recentOutcomeType = 'investigate'; maybeStageAdvance();
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.isCrit) {
        ...
        addJournal('investigation', 'Guildheart zero-rated Panim memorial imports — 22x audit threshold, retroactive exemption', `guild-derris-${G.dayCount}`);
      } else if (result.isFumble) {
        G.worldClocks.watchfulness = (G.worldClocks.watchfulness||0) + 1;
        ...
        addJournal('investigation', 'Zero-rated import category passed audit threshold — no review filed', `guild-derris-partial-${G.dayCount}`);
      }
      G.recentOutcomeType = 'investigate'; maybeStageAdvance();
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.isCrit) {
        ...
        addJournal('investigation', 'Guildheart inn: Iron Compact arbiter meets monthly-registered broker night before charter loads', `guild-cala-${G.dayCount}`);
      } else if (result.isFumble) {
        ...
        addJournal('complication', 'Innkeeper protective response — table flagged for watching', `guild-cala-fail-${G.dayCount}`);
      } else {
        G.flags.met_cala_ledgermere = true;
        G.investigationProgress++;
        ...
        addJournal('investigation', 'Regular off-channel arbiter: east arrival, northwest departure, sealed documents', `guild-cala-partial-${G.dayCount}`);
      }
      G.recentOutcomeType = 'investigate'; maybeStageAdvance();
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.isCrit) {
        ...
        addJournal('investigation', 'Shrine meeting: non-Windrider Roadwarden signed Collegium document — insignia confirmed', `guild-nyra-${G.dayCount}`);
      } else if (result.isFumble) {
        ...
        addJournal('complication', 'Shrine confidentiality invoked — alcove testimony refused, alternative paths offered', `guild-nyra-fail-${G.dayCount}`);
      } else {
        G.flags.met_nyra_ledgermere = true;
        G.investigationProgress++;
        ...
        addJournal('investigation', 'Shrine document exchange at charter exemption filing week', `guild-nyra-partial-${G.dayCount}`);
      }
      G.recentOutcomeType = 'investigate'; maybeStageAdvance();
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.total >= 14 || result.isCrit) {
        ...
        addJournal('investigation', 'Guildheart S2 finale: Union Guild Council charter audit initiated, contract rider voided', `guild-finale-inst-${G.dayCount}`);
      } else {
        ...
        addJournal('investigation', 'Guildheart S2 finale: charter evidence distributed to competing factions simultaneously', `guild-finale-uw-${G.dayCount}`);
      }
