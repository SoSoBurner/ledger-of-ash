...
const SUNSPIRE_HAVEN_TO_SHELK_ARC = [
...

  {
    ...
    fn: function() {
      ...
      addJournal('Left Sunspire Haven with container liner evidence', 'field_note', `sunspire-arc-departure-${G.dayCount}`);
    }
  },
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 10) {
        ...
        addJournal('Tennen: convoy guard rotation matches deviation dates — guards were managed', 'fact', `sunspire-arc-tennen-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      addJournal('Container is deployment-grade — designed for controlled release into dome systems', 'fact', `sunspire-arc-deployment-${G.dayCount}`);
      G.recentOutcomeType = 'success';
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.total >= 12) {
        ...
        if (!G.investigationProgress) G.investigationProgress = 0;
        G.investigationProgress++;
        addJournal('Three containers, three alias variants of same sender — 6 days ago, last passage', 'fact', `sunspire-arc-waybill-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 10) {
        ...
        addJournal('Dome atmospheric system is unified — single access point, full district propagation', 'fact', `sunspire-arc-dome-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 11) {
        ...
        if (!G.worldClocks) G.worldClocks = {};
        G.worldClocks.pressure = (G.worldClocks.pressure || 0) + 1;
        addJournal('Bookbinder: three prior Sunspire contacts never arrived — evidence now distributed', 'fact', `sunspire-arc-contact-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (!G.investigationProgress) G.investigationProgress = 0;
      G.investigationProgress++;
      addJournal('Arrived in Shelkopolis from Sunspire Haven — Stage 2 begins', 'fact', `sunspire-arc-finale-${G.dayCount}`);
      G.recentOutcomeType = 'success';
      maybeStageAdvance();
    }
  }
...
window.SUNSPIRE_HAVEN_TO_SHELK_ARC = SUNSPIRE_HAVEN_TO_SHELK_ARC;
