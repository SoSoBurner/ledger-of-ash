...
const SHIRSHAL_TO_SHELK_ARC = [
...

  {
    ...
    fn: function() {
      ...
      addJournal('Left Shirshal with ghost visitor records — six case officers disappeared administratively during Tazren case', 'field_note', `shirshal-arc-departure-${G.dayCount}`);
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.total >= 11) {
        ...
        addJournal('Undisposed case in Shelkopolis central bureau limbo files — someone keeping it accessible', 'fact', `shirshal-arc-case-${G.dayCount}`);
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 10) {
        ...
        if (!G.investigationProgress) G.investigationProgress = 0;
        G.investigationProgress++;
        addJournal('Wend refused rerouting order — license suspended; corridor/window matches ghost vessel dates', 'fact', `shirshal-arc-wend-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      addJournal('Ghost vessels and charter mark containers are parallel channels of same supply chain — both to Shelkopolis', 'fact', `shirshal-arc-maritime-${G.dayCount}`);
      G.recentOutcomeType = 'success';
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.total >= 12) {
        ...
        if (!G.investigationProgress) G.investigationProgress = 0;
        G.investigationProgress++;
        addJournal('Parro\'s clean reports match ghost vessel dates — deliberate non-reporting, transferred after last window', 'fact', `shirshal-arc-parro-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 10) {
        ...
        addJournal('Shirshal harbor maritime phase complete 17 days ago — supply chain delivered before the trail was followed to completion', 'fact', `shirshal-arc-harbor-${G.dayCount}`);
        G.recentOutcomeType = 'neutral';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 10) {
        ...
        addJournal('Ghost vessels used secondary freight terminal to underground channel — bypassed port inspection into interior distribution', 'fact', `shirshal-arc-port-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 11) {
        ...
        if (!G.worldClocks) G.worldClocks = {};
        G.worldClocks.pressure = (G.worldClocks.pressure || 0) + 1;
        addJournal('Aldeth: 6 ghost vessels as \'maintenance transit\' — queries rerouted to non-existent bureau', 'fact', `shirshal-arc-aldeth-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (!G.investigationProgress) G.investigationProgress = 0;
      G.investigationProgress++;
      addJournal('Arrived in Shelkopolis from Shirshal — Stage 2 begins', 'fact', `shirshal-arc-finale-${G.dayCount}`);
      G.recentOutcomeType = 'success';
      maybeStageAdvance();
    }
  }
