...
const HARVEST_CIRCLE_TO_SHELK_ARC = [
...

  {
    ...
    fn: function() {
      ...
      addJournal('Left Harvest Circle following charter mark supply chain south', 'field_note', `harvest-arc-departure-${G.dayCount}`);
    }
  },
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 10) {
        ...
        addJournal('Nann: count records being \'reviewed\' — she saved copies before leaving', 'fact', `harvest-arc-nann-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      addJournal('Routing anomaly is engineered offset — external system inserted into Harvest Circle logistics', 'fact', `harvest-arc-routing-${G.dayCount}`);
      G.recentOutcomeType = 'success';
    }
  },

  {
    ...
    fn: function() {
      ...
      if (result.total >= 11) {
        ...
        if (!G.investigationProgress) G.investigationProgress = 0;
        G.investigationProgress++;
        addJournal('Charter mark crates returned north empty — containers in circulation, not disposable', 'fact', `harvest-arc-charter-road-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 12) {
        ...
        if (!G.investigationProgress) G.investigationProgress = 0;
        G.investigationProgress++;
        addJournal('Veth Karst created non-existent transit category to exempt containers from inspection', 'fact', `harvest-arc-karst-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 10) {
        ...
        addJournal('Granary Steps connects to dome atmospheric distribution — terminus for charter mark supply chain', 'fact', `harvest-arc-granary-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (result.total >= 11) {
        ...
        if (!G.worldClocks) G.worldClocks = {};
        G.worldClocks.pressure = (G.worldClocks.pressure || 0) + 1;
        addJournal('Reth: four-piece network assembling in Shelkopolis — charter mark links all four', 'fact', `harvest-arc-reth-${G.dayCount}`);
        G.recentOutcomeType = 'success';
      } else {
...

  {
    ...
    fn: function() {
      ...
      if (!G.investigationProgress) G.investigationProgress = 0;
      G.investigationProgress++;
      addJournal('Arrived in Shelkopolis from Harvest Circle — Stage 2 begins', 'fact', `harvest-arc-finale-${G.dayCount}`);
      G.recentOutcomeType = 'success';
      maybeStageAdvance();
    }
  }
