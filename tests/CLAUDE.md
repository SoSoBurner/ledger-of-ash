...
- **Journal DOM structure**: `addJournal(text, category)` valid categories: `'evidence'`, `'intelligence'`, `'rumor'`, `'discovery'`, `'contact_made'`, `'complication'`, `'field_note'` — these are what `showJournal()` renders. Do NOT use `'fact'`, `'faction'`, `'quest'`, `'rival'`, `'companion'` — those are DOM section IDs in the wider journal overlay UI, not `addJournal()` category arguments. Validate with `tests/content/validate-structure.js` VALID_CATS. Rendered as `.jov-section` / `.jov-entry` divs. `#journal-overlay-body` holds all sections at once.
