  ...
  // 19. MORAL PRESSURE: COMPLICITY AND PARTICIPATION CHOICE
  {
    ...
    fn: function() {
      ...
      G.moralChoice = {
        ...
        expose: `Report ${npc.name}'s role in the displacement system. Disrupt the apparatus but lose insider access.`
      };

      addJournal('moral-choice', `Confronted ${npc.name} (${npc.role}) about displacement system participation`, `soreheim-moral-${G.dayCount}`);
  ...
  // 20. DISCOVERY MOMENT: EXTERNAL COORDINATION SOURCE
  {
    ...
    fn: function() {
      ...
      if (result.isCrit) {
        ...
        G.stageProgress[1]++;
        addJournal('major-discovery', 'Origin source of Soreheim workforce displacement identified as external coordination', `soreheim-origin-${G.dayCount}`);
      } else if (result.isFumble) {
        ...
        addJournal('major-discovery', 'External coordination of Soreheim displacement confirmed', `soreheim-origin-external-${G.dayCount}`);
      } else {
