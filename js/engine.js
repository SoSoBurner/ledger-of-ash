...
(function(){
  ...
  function defaultState(){
    ...
    return {
      ...
      alignmentSystem:{
        benevolence:0,order:0,
        baselineBenevolence:0,baselineOrder:0,
        originLocality:null,originArchetype:null,
        currentBandBenevolence:'center',currentBandOrder:'center',
        momentumBenevolence:0,momentumOrder:0,
        originDeviationBenevolence:0,originDeviationOrder:0,
        history:[]
      },
      heat:{
        min:-90,max:90,dailyDecayTowardZero:1,
        localities:{}
      },
      lawEnforcementPressure:{
        settlements:{}
      },
      civicSupportPressure:{
        settlements:{}
      },
      lawbreaking:{
        recentViolations:[],
        monthlyViolationCount:0,
        outstandingSubdualRisk:0,
        lastViolationDay:null
      },
      custody:{
        isImprisoned:false,
        jailStartDay:null,
        releaseDay:null,
        jailSettlementId:null,
        jailReason:null,
        sentenceDays:0,
        wasExecuted:false,
  ...
  // ── ALIGNMENT & HEAT SYSTEM ─────────────────────────────────────────────
  function initializeHeat(localityId){
    if(!G.heat.localities[localityId]){
      G.heat.localities[localityId]={
        current:0, recentChange:0, recentSources:[], 
        monthlyUnlawfulChoiceCount:0, monthlyLawfulAidCount:0,
        monthlyViolentResistanceCount:0, monthlyFailedEscapeCount:0,
        monthlyPublicViolationCount:0, monthlyProtectedEntryCount:0,
        lastHeatChangeDay:null
      };
    }
  }
  
  function initializeAlignment(originLocalityId, archetypeId){
    const locality = getLocality(originLocalityId);
    const archetype = getArchetype(archetypeId);
    
    const localityBase = locality && locality.alignmentBaseline ? 
      {benevolence: locality.alignmentBaseline.benevolence || 0, order: locality.alignmentBaseline.order || 0} :
      {benevolence: 0, order: 0};
    const archetypeMod = archetype && archetype.archetypeAlignmentModifier ? 
      {benevolence: archetype.archetypeAlignmentModifier.benevolence || 0, order: archetype.archetypeAlignmentModifier.order || 0} :
      {benevolence: 0, order: 0};
    
    // Weighting: 70% locality, 30% archetype
    G.alignmentSystem.benevolence = Math.round(localityBase.benevolence * 0.7 + archetypeMod.benevolence * 0.3);
    G.alignmentSystem.order = Math.round(localityBase.order * 0.7 + archetypeMod.order * 0.3);
    
    // Clamp to -50 to 50
    G.alignmentSystem.benevolence = Math.max(-50, Math.min(50, G.alignmentSystem.benevolence));
    G.alignmentSystem.order = Math.max(-50, Math.min(50, G.alignmentSystem.order));
    
    G.alignmentSystem.baselineBenevolence = G.alignmentSystem.benevolence;
    G.alignmentSystem.baselineOrder = G.alignmentSystem.order;
    G.alignmentSystem.originLocality = originLocalityId;
    G.alignmentSystem.originArchetype = archetypeId;
    
    updateAlignmentBands();
    initializeHeat(originLocalityId);
  }
  
  function updateAlignmentBands(){
    const b = G.alignmentSystem.benevolence;
    if(b >= 36) G.alignmentSystem.currentBandBenevolence = 'extreme-benevolent';
    else if(b >= 21) G.alignmentSystem.currentBandBenevolence = 'strong-benevolent';
    else if(b >= 8) G.alignmentSystem.currentBandBenevolence = 'lean-benevolent';
    else if(b >= -7 && b <= 7) G.alignmentSystem.currentBandBenevolence = 'center';
    else if(b >= -20) G.alignmentSystem.currentBandBenevolence = 'lean-cruel';
    else if(b >= -35) G.alignmentSystem.currentBandBenevolence = 'strong-cruel';
    else G.alignmentSystem.currentBandBenevolence = 'extreme-cruel';
    
    const o = G.alignmentSystem.order;
    if(o >= 36) G.alignmentSystem.currentBandOrder = 'extreme-order';
    else if(o >= 21) G.alignmentSystem.currentBandOrder = 'strong-order';
    else if(o >= 8) G.alignmentSystem.currentBandOrder = 'lean-order';
    else if(o >= -7 && o <= 7) G.alignmentSystem.currentBandOrder = 'center';
    else if(o >= -20) G.alignmentSystem.currentBandOrder = 'lean-anarchy';
  ...
  function applyAlignmentDrift(benevolenceDelta, orderDelta, heatDelta, localityId, source){
    ...
    updateAlignmentBands();
    if(benevolenceDelta > 0) G.alignmentSystem.momentumBenevolence += benevolenceDelta;
    if(orderDelta > 0) G.alignmentSystem.momentumOrder += orderDelta;
    G.alignmentSystem.originDeviationBenevolence = Math.abs(G.alignmentSystem.benevolence - G.alignmentSystem.baselineBenevolence);
    G.alignmentSystem.originDeviationOrder = Math.abs(G.alignmentSystem.order - G.alignmentSystem.baselineOrder);
    if(heatDelta !== 0 && localityId) applyLocalityHeat(localityId, heatDelta);
    G.alignmentSystem.history.push({day:G.dayCount, source, benevolenceDelta, orderDelta, heatDelta, localityId, resultingAlignment:{benevolence:G.alignmentSystem.benevolence, order:G.alignmentSystem.order}});
    if(G.alignmentSystem.history.length > 50) G.alignmentSystem.history.shift();
  }
  
  function applyLocalityHeat(localityId, delta){
    initializeHeat(localityId);
    const heat = G.heat.localities[localityId];
    heat.current = Math.max(-90, Math.min(90, heat.current + delta));
    heat.recentChange = delta;
    heat.lastHeatChangeDay = G.dayCount;
    if(delta > 0) heat.recentSources.push({day:G.dayCount, type:'hostile', amount:delta});
    else if(delta < 0) heat.recentSources.push({day:G.dayCount, type:'protective', amount:delta});
    if(heat.recentSources.length > 20) heat.recentSources.shift();
  }
  
  function dailyHeatDecay(){
    for(const localityId in G.heat.localities){
      const heat = G.heat.localities[localityId];
      if(heat.current > 0) heat.current = Math.max(0, heat.current - G.heat.dailyDecayTowardZero);
      else if(heat.current < 0) heat.current = Math.min(0, heat.current + G.heat.dailyDecayTowardZero);
    }
  }
  
  function getLocalityHeat(localityId){
    initializeHeat(localityId);
    return G.heat.localities[localityId].current;
  }
  
  function getLocalityHeatBand(localityId){
    const heat = getLocalityHeat(localityId);
    if(heat === 0) return 'neutral';
    if(heat > 0) return heat <= 9 ? 'watched' : heat <= 19 ? 'noticed' : heat <= 34 ? 'pursued' : heat <= 49 ? 'severe' : 'manhunt';
  ...
  function rotateAndLimitChoices(fullChoiceArray, stage){
    const baseChoices = fullChoiceArray.filter(c => c && !hasMechanicTag(c,'creature_combat'));
    const tacticalCombatChoices = fullChoiceArray.filter(c => c && hasMechanicTag(c,'creature_combat'));
  ...
  function checkCompanionLeaveConditions(){
    ...
    G.companions=G.companions.filter(comp=>{
      ...
      const highHeat=(getCurrentLocalityHeat()>=8);
  ...
  function applyLegalityShift(delta){
    applyLocalityHeat(G.location, delta);
    const newHeat=getCurrentLocalityHeat();
    if(newHeat>=8 && G.legalityState && !G.legalityState.warrants.includes('active_warrant')){
      G.legalityState.warrants.push('active_warrant');
      addNotice('An active warrant has been issued for your character.');
    }
  }

  // ── SETTLEMENT PRESSURE ─────────────────────────────────
  function evaluateSettlementPressure(localityId){
    initializeHeat(localityId);
    const heat=G.heat.localities[localityId];
    const current=heat.current;
    const as=G.alignmentSystem||{benevolence:0,order:0};
    const anarchy=Math.max(0,-as.order);
    const cruel=Math.max(0,-as.benevolence);
    const monthlyHostile=(heat.monthlyUnlawfulChoiceCount||0)+(heat.monthlyViolentResistanceCount||0)+(heat.monthlyPublicViolationCount||0);
    const monthlyFriendly=(heat.monthlyLawfulAidCount||0)+(heat.monthlyProtectedEntryCount||0);
    const warrantActive=G.legalityState&&G.legalityState.warrants&&G.legalityState.warrants.includes('active_warrant');
    const enfScore=current*2+(anarchy*0.8)+(cruel*0.4)+monthlyHostile*3+(warrantActive?20:0);
    const supScore=(-current)+(Math.max(0,as.order)*0.5)+(Math.max(0,as.benevolence)*0.5)+monthlyFriendly*2;
    if(supScore>15 && current<=-8) return {type:'support',level:supScore>30?'strong':'mild'};
    if(enfScore>=40||warrantActive) return {type:'enforcement',level:'arrest'};
    if(enfScore>=20) return {type:'enforcement',level:'confrontation'};
    if(enfScore>=10) return {type:'scrutiny',level:'mild'};
    return {type:'none'};
  }

  function maybeActivateEnforcement(){
    if(G.enforcementState) return;
    if(G.combatSession||G.encounter||G.postCombatResolution) return;
    if(G.enforcementCooldown && G.dayCount < G.enforcementCooldown) return;
    const pressure=evaluateSettlementPressure(G.location);
    if(pressure.type!=='enforcement') return;
    const chance=pressure.level==='arrest'?0.55:0.32;
    if(Math.random()<chance){
      G.enforcementState={level:pressure.level,localityId:G.location,day:G.dayCount};
      const msg=pressure.level==='arrest'?
        'Authority figures are moving to intercept you. An arrest attempt is in progress.':
        'Official attention has found you. A confrontation is developing.';
      addNotice(msg,'urgent');
      G.lastResult=msg;
    }
  }

  function enforcementChoices(){
    const heat=getCurrentLocalityHeat();
    const as=G.alignmentSystem||{benevolence:0,order:0};
    const deepMisaligned=as.benevolence<-35||as.order<-35;
    const isStage45=G.stage>=4;
    const level=G.enforcementState&&G.enforcementState.level||'confrontation';

    function resolveEnforcement(){ G.enforcementState=null; G.enforcementCooldown=G.dayCount+10; }

    return [
      {
        label:'Submit to the authority and accept the process',
        tags:['Lawful','Submission'],mechanicTags:['subdual'],
        fn(){
          if(deepMisaligned&&!isStage45){
            G.hp=0;
            G.lastResult='The authority determined the charges merited final resolution. The ledger closes here.';
            G.recentOutcomeType='combat_execute';
            addNotice('Executed by authority order.');
          } else {
            const jailDays=deepMisaligned&&isStage45?180:Math.min(90,Math.max(1,Math.floor(heat*2.5)));
            advanceTime(jailDays);
            applyLocalityHeat(G.location,-Math.floor(heat*0.6));
            if(G.legalityState) G.legalityState.warrants=G.legalityState.warrants.filter(w=>w!=='active_warrant');
            G.lastResult=`${jailDays} days of custody. The sentence is served. The warrant is lifted — the heat has not fully cleared.`;
            G.recentOutcomeType='civic_lawful';
            addNotice(`Custody: ${jailDays} days served. Warrant lifted.`,'stage');
          }
          resolveEnforcement();
        }
      },
      {
        label:'Identify yourself and request authorized passage',
        tags:['Lawful','Negotiation'],mechanicTags:['resolution'],
        fn(){
          advanceTime(1);
          const bonus=(G.skills.persuasion||0)+Math.max(0,Math.floor(-heat/3));
          const roll=rollD20('persuasion',bonus);
          const target=10+Math.max(0,Math.floor(heat*0.8));
          G.lastRoll={action:'Request authorized passage',skill:'persuasion',total:roll.total,target,success:roll.isCrit||(roll.total>=target&&!roll.isFumble),die:roll.die,crit:roll.isCrit,fumble:roll.isFumble};
          if(G.lastRoll.success){
            applyLocalityHeat(G.location,-2);
            G.lastResult='The identification and account was accepted. Passage authorized with reduced scrutiny.';
            G.recentOutcomeType='civic_lawful';
            addNotice('Negotiated passage — heat reduced.','world');
          } else {
            applyLocalityHeat(G.location,1);
            G.lastResult='The request did not satisfy the authority. Scrutiny increased.';
      ...
      {
        label:'Break past before the closure completes',
        tags:['Chaotic','Stealth'],mechanicTags:['risk'],
        fn(){
          advanceTime(1);
          const bonus=G.skills.stealth||0;
          const roll=rollD20('stealth',bonus);
          const target=12+Math.max(0,Math.floor(heat*0.6));
          G.lastRoll={action:'Break past enforcement closure',skill:'stealth',total:roll.total,target,success:roll.isCrit||(roll.total>=target&&!roll.isFumble),die:roll.die,crit:roll.isCrit,fumble:roll.isFumble};
          if(G.lastRoll.success){
            G.lastResult='Distance and angle were enough. The pursuit dissolved before it organized.';
            G.recentOutcomeType='class_stealth';
            addNotice('Slipped enforcement closure.','world');
          } else {
            applyLocalityHeat(G.location,roll.isFumble?5:3);
            const h=G.heat.localities[G.location]; if(h) h.monthlyFailedEscapeCount=(h.monthlyFailedEscapeCount||0)+1;
            G.lastResult='The attempt to clear the area failed. The authority now has cause to escalate.';
            G.recentOutcomeType='civic_chaotic';
          }
          resolveEnforcement();
        }
      ...
      {
        label:'Force through the enforcement line',
        tags:['Chaotic','Combat','Confrontation'],mechanicTags:['combat','risk'],
        fn(){
          applyLocalityHeat(G.location,4);
          const h=G.heat.localities[G.location]; if(h) h.monthlyViolentResistanceCount=(h.monthlyViolentResistanceCount||0)+1;
          G.lastResult='The confrontation with authority escalated to violence. The heat is now severe.';
          G.recentOutcomeType='combat_start';
          resolveEnforcement();
          startThreatEncounter('enforcement');
        }
      }
    ];
  }

  function checkCivicSupport(){
    const heat=getCurrentLocalityHeat();
    const as=G.alignmentSystem||{benevolence:0,order:0};
    if(heat>-8) return null;
    if(as.order<10 && as.benevolence<10) return null;
    if(heat<=-35) return 'The authority here knows your work. They clear the way and offer assistance without being asked.';
    if(heat<=-20) return 'A local official nods recognition. The inspection is brief and the result is favorable.';
    return 'The checkpoint gives you an easier pass than most people receive here.';
  }

  function monthlyLocalityReset(){
    for(const id in G.heat.localities){
      const h=G.heat.localities[id];
      if(!h||typeof h!=='object') continue;
      h.monthlyUnlawfulChoiceCount=0;
      h.monthlyLawfulAidCount=0;
      h.monthlyViolentResistanceCount=0;
      h.monthlyFailedEscapeCount=0;
      h.monthlyPublicViolationCount=0;
      h.monthlyProtectedEntryCount=0;
    }
  }
  ...
  // ── TYPED TAG SYSTEM ────────────────────────────────────
  // Converts flat legacy tags[] into typed {themeTags, mechanicTags, routingTags}
  function normalizeChoiceTags(choice){
    if(choice._tagsNormalized) return choice;
    const flat=choice.tags||[];
    const theme=[...(choice.themeTags||[])];
    const mechanic=[...(choice.mechanicTags||[])];
    const routing=[...(choice.routingTags||[])];
    const add=(arr,v)=>{ if(!arr.includes(v)) arr.push(v); };
    for(const t of flat){
      switch(t){
        case 'Combat': case 'Direct': case 'Boss': add(mechanic,'combat'); break;
        case 'CreatureCombat': add(mechanic,'creature_combat'); break;
        case 'Risk': case 'Risky': case 'Confrontation': add(mechanic,'risk'); break;
        case 'ForceTacticalUI': add(routing,'force_tactical_ui'); break;
        case 'Final': add(theme,'finale'); break;
        case 'Institutional': add(theme,'institutional'); break;
        case 'Investigation': add(mechanic,'investigation'); break;
        case 'Assassination': add(mechanic,'combat'); add(theme,'criminal'); break;
        case 'Faction': add(mechanic,'faction_shift'); break;
        case 'Travel': add(mechanic,'travel'); break;
        case 'Rest': add(mechanic,'rest'); break;
        case 'Meaningful': add(mechanic,'resolution'); break;
      }
    }
    return {...choice, themeTags:theme, mechanicTags:mechanic, routingTags:routing, _tagsNormalized:true};
  }
  function hasThemeTag(choice,tag){ return normalizeChoiceTags(choice).themeTags.includes(tag); }
  function hasMechanicTag(choice,tag){ return normalizeChoiceTags(choice).mechanicTags.includes(tag); }
  function hasRoutingTag(choice,tag){ return normalizeChoiceTags(choice).routingTags.includes(tag); }

  function buildChoiceTagPills(choice, riskTag){
    const nc=normalizeChoiceTags(choice);
    const cp=normalizeChoiceProfile(choice);
    const pills=[];
    const tcap=s=>s?s.charAt(0).toUpperCase()+s.slice(1):'';

    // 1. Risk
    const riskCls=riskTag==='Safe'?'good':riskTag==='Risky'?'bad':'warn';
    pills.push(`<span class='riskPill riskPill-${riskCls}'>${riskTag}</span>`);

    // 2. Skill method (omit survival — it's the default, not informative)
    if(cp.skill && cp.skill!=='survival'){
      const sl={combat:'Combat',lore:'Lore',stealth:'Stealth',craft:'Craft',persuasion:'Persuasion'};
      pills.push(`<span class='riskPill'>${sl[cp.skill]||tcap(cp.skill)}</span>`);
    }

    // 3. Order / Anarchy axis
    if(cp.axis.civic>0) pills.push(`<span class='riskPill' style='border-color:#204858;color:#70a0c8'>Order</span>`);
    else if(cp.axis.civic<0) pills.push(`<span class='riskPill' style='border-color:#5a3810;color:#d09050'>Anarchy</span>`);

    // 4. Benevolence / Cruelty axis
    if(cp.axis.moral>0) pills.push(`<span class='riskPill' style='border-color:#1a4828;color:#70a878'>Benevolent</span>`);
    else if(cp.axis.moral<0) pills.push(`<span class='riskPill' style='border-color:#601010;color:#d06060'>Cruel</span>`);

    // 5. Archetype hook
    if((choice.tags||[]).includes('Class')) pills.push(`<span class='riskPill'>Archetype</span>`);

    // 6. Notable mechanic tags
    const showMech=['investigation','faction_shift','subdual','hazard','travel','rest'];
    for(const t of nc.mechanicTags){
      if(showMech.includes(t)) pills.push(`<span class='riskPill'>${tcap(t.replace(/_/g,' '))}</span>`);
    }

    // 7. Notable theme tags (plot significance)
    const showTheme=['finale','institutional','criminal','ritual','religious'];
    for(const t of nc.themeTags){
      if(showTheme.includes(t)) pills.push(`<span class='riskPill' style='border-color:#5a3820;color:#c09060'>${tcap(t)}</span>`);
    }
  ...
  function computeSceneState(){
    ...
    return {
      ...
      enforcementState:G.enforcementState||null,
      heatBand:getLocalityHeatBand(G.location)
    };
  }
  ...
  function stage2Choices(){ const sig=routeSignature(); const atlas=routeAtlasFor(sig);
    ...
    // Add enriched Stage 2 choices (45 total)
    const enrichedStage2Choices = getEnrichedStage2Choices();
    if (enrichedStage2Choices.length > 0) {
      choices.push(...enrichedStage2Choices);
    }
  ...
  function currentChoices(){
    ...
    if(G.enforcementState) return enforcementChoices();
    maybeActivateEnforcement();
    if(G.enforcementState) return enforcementChoices();
  ...
  function renderChoices(){
    ...
    $('choices').innerHTML=list.map((c,i)=>{
      ...
      const nc=normalizeChoiceTags(c);
      ...
      const isCombat=hasMechanicTag(c,'creature_combat')||hasMechanicTag(c,'combat')&&(c.tags||[]).includes('Assassination');
      const extraCls=(isClass?' classChoice':'')+(isEvil?' evilChoice':'')+(isGood?' goodChoice':'')+(isLawful?' lawfulChoice':'')+(isCombat?' combatChoice':'');
      const pills=buildChoiceTagPills(c,risk);
      return `<button class='choice${extraCls}' data-choice='${i}'><span>${c.label}</span><small>${pills}</small></button>`;
    }).join('');
    [...document.querySelectorAll('[data-choice]')].forEach(btn=>btn.onclick=()=>{
      ...
      // Stealth rule: capture order state before execution
      const isStealth=cp.skill==='stealth';
      const preOrder=G.alignmentSystem?G.alignmentSystem.order:0;

      c.fn();

      // Stealth drift rule (post-execution)
      if(isStealth && G.lastRoll && G.alignmentSystem){
        const orderDrift=G.alignmentSystem.order-preOrder;
        if(orderDrift!==0){
          if(G.lastRoll.success){
            // Stealth success: cancel Order/Anarchy drift
            G.alignmentSystem.order=preOrder;
            updateAlignmentBands();
          } else if(!G.lastRoll.success && orderDrift<0){
            // Stealth failure: double Anarchy drift
            G.alignmentSystem.order=Math.max(-50,preOrder+orderDrift*2);
            updateAlignmentBands();
          }
        }
      }

      // Combat initiation heat: apply locality heat based on motive when routing to combat
      if(cp.combatRouting && !G.combatSession){
        if(cp.axis.civic>0) applyLocalityHeat(G.location,-1);      // lawful/protective: earns civic trust
        else if(cp.axis.civic<0) applyLocalityHeat(G.location,2);  // anarchic: draws scrutiny
        else applyLocalityHeat(G.location,1);                       // neutral combat: some attention
        // Track monthly public violation if anarchic
        if(cp.axis.civic<0){ const h=G.heat.localities[G.location]; if(h) h.monthlyPublicViolationCount=(h.monthlyPublicViolationCount||0)+1; }
      }
  ...
  function render(){
    ...
    let sig={}, atlas={};
    try{ sig=routeSignature()||{}; atlas=routeAtlasFor(sig)||{}; }catch(e){}
    const _loc=getLocality(G.location); const _timeLabel=['Dawn','Morning','Midday','Late Day','Night'][G.timeIndex]; const _heatBadge=(getCurrentLocalityHeat()>0)?`<span style='color:#c27f7f;margin-left:6px'>Heat ${getCurrentLocalityHeat()}</span>`:''; const _combatBadge=G.combatSession?`<span style='color:#c9a46b;margin-left:6px'>IN COMBAT</span>`:''; $('header').innerHTML=`<span style='color:var(--accent)'>${_loc.name}</span> · Day ${G.dayCount} · ${_timeLabel} · Lvl ${G.level} · ${G.stageLabel} · ${_loc.polity||''}${_heatBadge}${_combatBadge}`;
  ...
  function beginNew(){
    ...
    initializeAlignment(G.location, G.archetype);
  ...
  function loadLegend(){ 
    ...
    if(!found){ alert('Legend not found.'); return; } 
    G=found; 
    if(!G.familyEdges) G.familyEdges=[]; 
    if(!G.telemetry) G.telemetry={turns:0,actions:0,travels:0,scouts:0,encounters:0,wins:0,rescues:0,services:0}; 
    if(!G.inventory) G.inventory=[]; 
    if(!G.equipment) G.equipment={}; 
    if(!G.serviceLog) G.serviceLog=[]; 
    if(!G.alignment) G.alignment={goodEvil:0,lawfulChaotic:0}; 
    // Migrate old civicHeat to new locality heat system
    if(!G.heat) G.heat={min:-90,max:90,dailyDecayTowardZero:1,localities:{}};
    if(!G.heat.localities) G.heat.localities={};
    if(!G.heat.localities[G.location] && G.legalityState && G.legalityState.civicHeat) {
    ...
    for(const id in G.heat.localities){
      if(typeof G.heat.localities[id]==='number'){
        const val=G.heat.localities[id];
        G.heat.localities[id]={current:val,recentChange:0,recentSources:[],monthlyUnlawfulChoiceCount:0,monthlyLawfulAidCount:0,monthlyViolentResistanceCount:0,monthlyFailedEscapeCount:0,monthlyPublicViolationCount:0,monthlyProtectedEntryCount:0,lastHeatChangeDay:null};
      }
    }
    render(); 
    const ss=$('startScreen'); if(ss) ss.style.display='none'; 
  ...
  // ── EXPORT ALIGNMENT SYSTEM TO WINDOW ──
  window.alignmentSystemExports = {
    initializeAlignment,
    applyAlignmentDrift,
    applyLocalityHeat,
    getLocalityHeat,
  ...
  function shouldSkipChoice(choiceLabel, choiceTags) {
    ...
    const isCombatChoice = hasMechanicTag({tags:choiceTags},'combat') || hasMechanicTag({tags:choiceTags},'creature_combat');
    if(isCombatChoice && recentChoices.length > 0){
      const lastChoice = recentChoices[recentChoices.length - 1];
      if(lastChoice.tags && (hasMechanicTag({tags:lastChoice.tags},'combat') || hasMechanicTag({tags:lastChoice.tags},'creature_combat'))){
        return true;
      }
    }
  ...
  window.hasRoutingTag = hasRoutingTag;
