...
(function(){
  ...
  function composeImmediateResponseLayer(ss, gs){
    ...
    if(t === 'combat_start') return `A confrontation forces itself into the foreground.`;
    if(t === 'combat_ongoing') return `The fight continues. Round ${gs.combatSession?gs.combatSession.round:'—'}.`;
    if(t === 'combat_victory') return `The confrontation resolved. The space holds a different weight now.`;
    if(t === 'combat_flee') return `Distance gained. The threat did not follow, or chose not to.`;
    if(t === 'combat_mercy') return `The merciful choice leaves its own mark on how the space reads.`;
    if(t === 'combat_execute') return `The execution closes one door and opens a longer account.`;
    if(t === 'combat_interrogate') return `Interrogation yields something the record did not show.`;
    if(t === 'combat_loot') return `Dealt with efficiently. Gold and evidence secured.`;
    if(t === 'moral_good') return `The intervention left a mark on the place and on the ledger.`;
    if(t === 'moral_evil') return `The gain is real. The tab is longer than it looks.`;
    if(t === 'civic_lawful') return `The legitimate path cost time but cost less else.`;
    if(t === 'civic_chaotic') return `The bypass worked, at least this time. The official channels are watching.`;
    if(t === 'observe') return `A quieter read begins to surface what the usual view misses.`;
    if(t === 'class_combat') return `Martial discipline surfaces something the investigative path would have missed.`;
    if(t === 'class_magic') return `Arcane attention reveals layered pressures beneath the presenting surface.`;
    if(t === 'class_stealth') return `The unobserved angle opens a different kind of picture.`;
    if(t === 'class_support') return `Practical engagement shifts who is available and what they will say.`;
    return last;
  }
